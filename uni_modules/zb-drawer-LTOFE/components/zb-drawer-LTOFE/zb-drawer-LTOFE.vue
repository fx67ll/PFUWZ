<template>
	<view @touchmove.stop.prevent="() => {}" <!-- #ifdef H5 -->
		...
		v-if="show"
		<!-- #endif -->
		class="zb-drawer-LTOFE__wrapper" >
		...
		<view
			...
			@touchmove="touchmove"
			@click.stop=""
			class="zb-drawer-LTOFE__container"
			:class="[
				`zb-drawer-LTOFE_${mode}`,
				{
					animation: isMove,
		...
		getDrawerWidth() {
			this.$nextTick(() => {
				uni.createSelectorQuery()
					.in(this)
					.select('.zb-drawer-LTOFE__container')
...
.zb-drawer-LTOFE_bottom {
	bottom: 0;
	right: 0;
	left: 0;
	transform: translateY(100%);
}
.zb-drawer-LTOFE_right {
	right: 0;
	top: 0;
	bottom: 0;
	transform: translateX(100%);
}
.zb-drawer-LTOFE_left {
	top: 0;
	left: 0;
	bottom: 0;
	transform: translateX(-100%);
